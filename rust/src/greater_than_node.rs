...
use crate::instruction_serialiser::{ArithmeticResultNodeWrapper, GreaterThanNode, LogicalResultNodeWrapper};
use crate::instruction_serialiser::logical_result_node_wrapper::Node;
use crate::logical_type::LogicalType;
use crate::node_wrapper::NodeWrapper;
use crate::parameter::Parameter;

impl GenericNode<LogicalType> for GreaterThanNode {
    fn calculate(&self, parameters: &HashMap<&str, Parameter>) -> Result<LogicalType, CalculationError> {
        let left_node = self.left_child.as_ref().ok_or(
            CalculationError::new("The left child of a greater than node must be present to perform a calculation")
        )?;
        let right_node = self.right_child.as_ref().ok_or(
            CalculationError::new("The right child of a greater than node must be present to perform a calculation")
        )?;
...
impl ArithmeticResultNodeWrapper {
    pub fn greater_than(self, node: ArithmeticResultNodeWrapper) -> LogicalResultNodeWrapper {
